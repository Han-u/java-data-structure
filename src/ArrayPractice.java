...
class MyArray{
    ...
    public void insertData(int index, int data){
        ...
        int[] arrDup = this.arr.clone(); // 기존 배열 복사
        ...
        this.arr[index] = data; // 새로운 데이터 삽입
    }
    ...
    public void removeData(int data){
        ...
        for (int i = 0; i < this.arr.length; i++) {
            if(this.arr[i] == data){ // 해당 내용 있는지 확인
                targetIndex = i;
                break;
            }
        }
        ...
        if(targetIndex == -1){
            ...
            int[] arrDup = this.arr.clone(); // 배열 백업
            this.arr = new int[this.arr.length - 1];
...
public class ArrayPractice {
    public static void main(String[] args){
        ...
        myArray.insertData(-1, 0); //IndexError
